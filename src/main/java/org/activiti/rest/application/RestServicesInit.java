...
import com.cbs.rest.api.engine.ProcessEngineResourceCBS;
import com.cbs.rest.api.form.ValidateFormPropertiesResource;
import com.cbs.rest.api.history.ProcessInstanceActivitiesResource;
import com.cbs.rest.api.history.ProcessInstanceTreeActivityCollectionResource;
import com.cbs.rest.api.history.TaskHistoricFormPropertyResource;
import com.cbs.rest.api.indentity.LoginUserAndGroupInfoResource;
import com.cbs.rest.api.process.MessageAdHocProcessResource;
import com.cbs.rest.api.process.MessageReceivedResource;
import com.cbs.rest.api.process.ProcessCommentCollectionResource;
import com.cbs.rest.api.process.ProcessCommentResource;
import com.cbs.rest.api.process.ProcessInstanceAdhocResource;
import com.cbs.rest.api.process.ProcessInstanceResourceCBS;
import com.cbs.rest.api.process.ProcessInstanceVariableContactResource;
import com.cbs.rest.api.process.ProcessInstanceVariableResourceCBS;
import com.cbs.rest.api.process.ProcessInstancesActivityCollectionNativeResource;
import com.cbs.rest.api.process.ProcessInstancesActivityCollectionResource;
import com.cbs.rest.api.process.SignalReceivedResource;
import com.cbs.rest.api.process.SubProcessInstancesActivityCollectionResource;
import com.cbs.rest.api.process.SubprocessInstanceDiagramResource;
import com.cbs.rest.api.repository.ProcessDefinitionLastDeploymentCollectionResource;
import com.cbs.rest.api.task.TaskCollectionResourceCBS;
import com.cbs.rest.api.task.TaskCollectionSummaryResource;
import com.cbs.rest.api.task.TaskDocumentationResource;
import com.cbs.rest.api.task.TaskOperationResourceCBS;
import com.cbs.rest.api.task.TaskPropertiesResourceCBS;
import com.cbs.rest.api.task.TaskResourceCBS;
import com.cbs.rest.api.task.TaskResourceDefinitionKey;
import com.cbs.rest.api.task.TaskResourceHistoric;

public class RestServicesInit {

  public static void attachResources(Router router) {
	// Rest API URLs mapping customized by CBS
	router.attach("/cbs/process-engine", ProcessEngineResourceCBS.class);
    // get processes with tasks and subprocesses
    router.attach("/cbs/runtime/process-instances/activities", ProcessInstancesActivityCollectionResource.class);
    // get subprocesses with tasks and subprocesses
    router.attach("/cbs/runtime/subprocess-instances/activities", SubProcessInstancesActivityCollectionResource.class);
    // get processes with activities using native query, process-type can be "in-progress" or "candidate"
    router.attach("/cbs/runtime/process-instances/native/{process-type}", ProcessInstancesActivityCollectionNativeResource.class);
    // historic form properties input values 
    router.attach("/cbs/history/{taskId}/form-properties", TaskHistoricFormPropertyResource.class);
    // retrieve process instance and its task list
    router.attach("/cbs/history/process-instances/{processInstanceId}", ProcessInstanceActivitiesResource.class);
    // login with user group information returned
    router.attach("/cbs/identity/login", LoginUserAndGroupInfoResource.class);
    // active task list sorted in specified conditions
    router.attach("/cbs/runtime/tasks", TaskCollectionResourceCBS.class);
    // Task summary list of all users in specified conditions
    router.attach("/cbs/runtime/tasks/summary/assignees", TaskCollectionSummaryResource.class);
    // apart from default operation, "in progress" concept is supported
    router.attach("/cbs/runtime/task/{taskId}/{operation}", TaskOperationResourceCBS.class);
    // return active task response with additional information such as in-progress time 
    router.attach("/cbs/runtime/task/{taskId}", TaskResourceCBS.class);
    router.attach("/cbs/runtime/tasks/{taskId}", TaskResourceCBS.class);
    // get task by process id and task definition key
    router.attach("/cbs/runtime/task/{processId}/definition/{taskDefinitionKey}", TaskResourceDefinitionKey.class);
    // return historic task response with additional information such as in-progress time 
    router.attach("/cbs/runtime/task-historic/{taskId}", TaskResourceHistoric.class);
    // form properties including CBS customized types such as option
    router.attach("/cbs/form/{taskId}/properties", TaskPropertiesResourceCBS.class);
    // get all process definitions from latest deployment
    router.attach("/cbs/repository/process-definitions", ProcessDefinitionLastDeploymentCollectionResource.class);
    // retrieve task documentation
    router.attach("/cbs/repository/task/{taskId}/documentation", TaskDocumentationResource.class);
    // validate task form user inputs and complete task form if pass validation
    router.attach("/cbs/runtime/form-properties/{taskId}/validate", ValidateFormPropertiesResource.class);
    // perform actions on process such as enlist/unlist candidate, watch/unwatch process, activate/suspend process
    router.attach("/cbs/runtime/process-instances/{processInstanceId}", ProcessInstanceResourceCBS.class);
    // process comments of single process
    router.attach("/cbs/runtime/process-instances/{processInstanceId}/comments", ProcessCommentResource.class);
    // process comments of whole process tree 
    router.attach("/cbs/runtime/process-instance-tree/{processInstanceId}/comments", ProcessCommentCollectionResource.class);
    // update process variables based on Saasu contact uid
    router.attach("/cbs/runtime/process-instances/contact/{contactuid}/{action}", ProcessInstanceVariableContactResource.class);
    // send signal to process
    router.attach("/cbs/runtime/process-instance/{processInstanceId}/signal", SignalReceivedResource.class);
    // send message to execution
    router.attach("/cbs/runtime/executions/{executionId}/message", MessageReceivedResource.class);

    // added in Activiti 5.13, to be removed 
    router.attach("/cbs/runtime/process-instance/{processInstanceId}/variable", ProcessInstanceVariableResourceCBS.class);
    
    // tesing purpose only
    router.attach("/cbs/runtime/subprocess-instance/diagram", SubprocessInstanceDiagramResource.class);

    // beta, call Ad-Hoc process to create task or call activites(subprocess)
    router.attach("/cbs/runtime/process-instances-adhoc/{adhocProcessId}/message", MessageAdHocProcessResource.class);
    
    // beta, 
    router.attach("/cbs/runtime/process-instances-adhoc", ProcessInstanceAdhocResource.class);
    
    // beta, retrieve process instance tree with task and call activities of each process
    router.attach("/cbs/history/process-instance-tree/{processInstanceId}", ProcessInstanceTreeActivityCollectionResource.class);
//    http://192.168.1.36:8081/activiti-rest-cbs/service/cbs/history/process-instance-tree/136716

    // Activiti Rest API URLs stay untouched 
    // New REST-urls
